...
class DisclaimerByDomain(IMapiSpoolerPlugin):
    ...
    def getSignature(self, extension, mail):
        # enter here the command that should be executed (shell)
        command = 'php /opt/postfix-tools/signature.php "' + mail + '" ' + extension

        # converts command into a list for Popen
        command_converted = shlex.split(command)

        # executes command and returns stdout and stderr pipe
        process = Popen(command_converted, stdout=PIPE, stderr=PIPE)
    ...
    def PreSending(self, session, addrbook, store, folder, message):
        ...
        if bodytag == PR_BODY_W:
            ...
            signature = "<br>" + self.getSignature('html', sender_email_address)
            ...
            self.logger.logWarn("!--- RTF signature is not supported")
            # RTF not supported because this cause body issues.
            return MP_CONTINUE,

            rtf = self.getSignature('rtf', sender_email_address)
