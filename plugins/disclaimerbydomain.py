...
class DisclaimerByDomain(IMapiSpoolerPlugin):
    ...
    def PreSending(self, session, addrbook, store, folder, message):
        ...
        if len(sender_email_address_property[0].Value) > 0:
            sender_email_address = sender_email_address_property[0].Value.encode("utf-8")
            self.logger.logDebug("*--- Sender mail address is '%s'" % (sender_email_address))
            company = sender_email_address.split('@')[1]
            self.logger.logDebug("*--- Domain of company is '%s'" % (company))
